...
void receiverLoop(u32 param) {
    ...
    mod::ConsoleWindow::init();
    mod::ConsoleWindow::push("Initializing network...\n");

    s32 res = initializeNetwork();

    if (res < 0) {
        CONSOLE_PUSH_FMT("Initializing network failed. %d\n", res);
        return;
    }
    else {
        mod::ConsoleWindow::push("Network initialized successfully.");
    }
